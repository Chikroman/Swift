...
class NewsTableViewCell: UITableViewCell {
    ...
    func configure(feed: FactoryNewModel) {
        
        labelDate.text = feed.labelDateText //.getFeedDate()
        labelFeedGroupHeader.text = feed.labelFeedGroupHeaderText //sourceName
        ...
        labelLike.text = feed.labelLikeText // feed.getStringFrom(count: feed.countLikes)
        labelViews.text = feed.labelViewsText // feed.getStringFrom(count: feed.countViews)
        labelShare.text = feed.labelShareText // feed.getStringFrom(count: feed.countReposts)
        labelComment.text = feed.labelCommentText // feed.getStringFrom(count: feed.countComments)
        
        imageViewGroup.sd_setImage(with: URL(string: feed.sourceUrl), placeholderImage: UIImage(named: "noPhoto"))
        
        if feed.feed.attachments.count > 0 {
